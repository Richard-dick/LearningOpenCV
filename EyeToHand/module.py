...
K=np.array([[896.29746575,  0., 649.74499104]
...
#用来从棋盘格图片得到相机外参
def get_RT_from_chessboard(img_path:str, width:int, height:int, K, len) -> np.array:
    '''
    :param img_path: 读取图片路径
    :param width: 棋盘格x方向格子数
    :param height: 棋盘格y方向格子数
    :param K: 相机内参
    :param len: 单位棋盘格长度,mm
    :return: 相机外参
    '''
    ...
    object_points[:, :2] = np.mgrid[0:height, 0:width].T.reshape(-1, 2)
    object_points = len * object_points  
    # object_points=np.zeros((3,width*height),dtype=np.float64)
    # flag=0
    # for i in range(height):
    #     for j in range(width):
    #         object_points[:2,flag]=np.array([(11-j-1)*len,(8-i-1)*len])
    #         flag+=1
    # print(object_points)
...
# folder = "calib"#棋盘格图片存放文件夹
# files = os.listdir(folder)
# file_num=len(files)
# RT_all=np.zeros((4,4,file_num))

# print(get_RT_from_chessboard('calib/2.bmp', width, height, K, len))
'''
...
for fname in images:
    RT=get_RT_from_chessboard(fname, width, height, K, len)
